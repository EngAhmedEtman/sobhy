<x-dynamic-component :component="request('print_mode') ? 'layouts.print' : 'layouts.app'" title="كشف حساب عميل">
    <x-slot name="breadcrumb">التقارير والإحصائيات / تقرير عميل</x-slot>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 mb-6 print:hidden">
        <form id="filterForm" action="{{ route('reports.customers') }}" method="GET" class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex-1 min-w-[130px]">
                ...
                <select name="transaction_type" class="w-full px-3 py-1.5 border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-all text-xs">
                    ...
                    <option value="sale" {{ request('transaction_type') == 'sale' ? 'selected' : '' }}>مبيعات فقط</option>
                    <option value="payment_received" {{ request('transaction_type') == 'payment_received' ? 'selected' : '' }}>دفعات مستلمة</option>
                </select>
            </div>
            <div class="flex-1 min-w-[140px]">
    ...
    @if($customer)
        ...
        <div class="hidden print:block mb-6">
            <x-print.header :title="'كشف حساب: ' . $customer->name" :subtitle="(request('start_date') || request('end_date')) ? 'الفترة من ' . (request('start_date') ?? 'البداية') . ' إلى ' . (request('end_date') ?? 'الآن') : 'جميع التعاملات'" />
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 mb-6 p-6 print:border-none print:shadow-none print:bg-transparent print:p-0 print:m-0">
            <div class="border-b border-slate-100 pb-4 mb-4 print:hidden">
                <h2 class="text-lg font-black text-slate-800">كشف حساب: {{ $customer->name }}</h2>
    ...
    @else
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-12 text-center flex flex-col items-center justify-center">
            ...
            <p class="text-sm text-slate-500">يرجى اختيار العميل من القائمة العلوية والضغط على "تطبيق" لاستعراض كشف الحساب التفصيلي.</p>
        </div>
    @endif
