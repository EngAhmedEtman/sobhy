...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'تقرير المشتريات الشامل']); ?>
     <?php $__env->slot('breadcrumb', null, []); ?> التقارير > مشتريات <?php $__env->endSlot(); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'تقرير المشتريات الشامل','subtitle' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(request('start_date') ? 'الفترة من: ' . request('start_date') . ' إلى: ' . (request('end_date') ?? 'اليوم') : 'التقرير الشامل')]); ?>
...
<?php endif; ?>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 print:hidden">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x lg:divide-x-reverse divide-slate-100">
            <div class="p-4 flex flex-col justify-center text-center hover:bg-slate-50 transition-colors rounded-r-xl">
                ...
                <span class="text-xl font-black text-slate-800" dir="ltr"><?php echo e(number_format($purchases->count())); ?></span>
            </div>
    ...
    <div class="hidden print:block mb-8">
        ...
        <table class="w-full text-center border-collapse">
            ...
            <tbody>
                <tr>
                    <td><?php echo e(number_format($purchases->count())); ?></td>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 mb-6 print:hidden">
        <form id="filterForm" action="<?php echo e(route('reports.purchases')); ?>" method="GET" class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                ...
                <a href="<?php echo e(route('reports.purchases')); ?>" class="flex-1 sm:flex-none px-5 py-1.5 text-xs font-bold text-slate-700 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-200 transition-colors text-center">تفريغ</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-center border-collapse whitespace-nowrap">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-slate-50/60 transition-colors group">
                        <td class="px-4 py-3 text-[0.8rem] font-bold text-slate-700 border-b border-slate-100"><?php echo e($purchase->invoice_number); ?></td>
                        <td class="px-4 py-3 text-[0.8rem] text-slate-600 border-b border-slate-100"><?php echo e(\Carbon\Carbon::parse($purchase->date)->format('Y-m-d')); ?></td>
                        <td class="px-4 py-3 text-[0.85rem] font-bold text-primary-700 border-b border-slate-100"><?php echo e($purchase->supplier->name ?? '-'); ?></td>
                        <td class="px-4 py-3 text-[0.85rem] text-slate-800 font-bold border-b border-slate-100" dir="ltr"><?php echo e(number_format($purchase->total_amount, 0)); ?></td>
                        <td class="px-4 py-3 text-[0.85rem] text-warning-600 font-bold border-b border-slate-100" dir="ltr"><?php echo e(number_format($purchase->paid_amount, 0)); ?></td>
                        <td class="px-4 py-3 text-[0.85rem] text-danger-600 font-bold border-b border-slate-100" dir="ltr"><?php echo e(number_format($purchase->remaining_amount, 0)); ?></td>
                    </tr>
