<x-dynamic-component :component="request('print_mode') ? 'layouts.print' : 'layouts.app'" title="تقرير المبيعات الشامل">
    <x-slot name="breadcrumb">التقارير > مبيعات</x-slot>
    ...
    <div class="hidden print:block mb-6">
        <x-print.header title="تقرير المبيعات الشامل" :subtitle="request('start_date') ? 'الفترة من: ' . request('start_date') . ' إلى: ' . (request('end_date') ?? 'اليوم') : 'التقرير الشامل'" />
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 print:hidden">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x lg:divide-x-reverse divide-slate-100">
            <div class="p-4 flex flex-col justify-center text-center hover:bg-slate-50 transition-colors rounded-r-xl">
                ...
                <span class="text-xl font-black text-slate-800" dir="ltr">{{ number_format($sales->count()) }}</span>
            </div>
    ...
    <div class="hidden print:block mb-8">
        ...
        <table class="w-full text-center border-collapse">
            ...
            <tbody>
                <tr>
                    <td>{{ number_format($sales->count()) }}</td>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 mb-6 print:hidden">
        <form id="filterForm" action="{{ route('reports.sales') }}" method="GET" class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                ...
                <a href="{{ route('reports.sales') }}" class="flex-1 sm:flex-none px-5 py-1.5 text-xs font-bold text-slate-700 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-200 transition-colors text-center">تفريغ</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-center border-collapse whitespace-nowrap">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-slate-50/60 transition-colors group">
                        ...
                        <td class="px-4 py-3 text-[0.85rem] text-danger-600 font-bold border-b border-slate-100" dir="ltr">{{ number_format($sale->remaining_amount, 0) }}</td>
                    </tr>
                    @empty
