<x-layouts.app title="إدارة العملاء">
    ...
    <div x-data="{ 
        ...
        fetchData(url = null) {
            this.loading = true;
            let endpoint = url || '{{ route('customers.index') }}';
            ...
            .then(html => {
                let wrapper = document.getElementById('customersTableWrapper');
    ...
    @click="if ($event.target.closest('.ajax-pagination a')) {
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
            ...
            <button @click="showAddModal = true" class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-xs sm:text-sm font-bold flex items-center transition-colors shadow-sm shadow-primary-600/20 w-full sm:w-auto justify-center">
        ...
        <div class="bg-white rounded-xl p-3 sm:p-4 border border-slate-100 shadow-sm mb-4">
            ...
            <div x-show="showFilters" 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2.5">
                    ...
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">حالة الحساب</label>
                        <select x-model="balance_status" @change="fetchData()" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500">
                            <option value="">جميع الحسابات</option>
                            <option value="debt">مطلوب منه (مديونية للشركة)</option>
                            <option value="credit">له رصيد (دافع زيادة)</option>
                    ...
                    <!-- Min Balance -->
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">من رصيد (مديونية)</label>
                        <input type="number" step="any" x-model="min_balance" @input.debounce.400ms="fetchData()" placeholder="0" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500 font-mono" dir="ltr">
                    </div>

                    <!-- Max Balance -->
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">إلى رصيد (مديونية)</label>
                        <input type="number" step="any" x-model="max_balance" @input.debounce.400ms="fetchData()" placeholder="مثال: 50000" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500 font-mono" dir="ltr">
                    </div>

                    <!-- Min Volume (Sales Total) -->
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">من حجم تعاملات</label>
                        <input type="number" min="0" step="any" x-model="min_volume" @input.debounce.400ms="fetchData()" placeholder="0" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500 font-mono" dir="ltr">
                    </div>

                    <!-- Max Volume (Sales Total) -->
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">إلى حجم تعاملات</label>
                        <input type="number" min="0" step="any" x-model="max_volume" @input.debounce.400ms="fetchData()" placeholder="مثال: 100000" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500 font-mono" dir="ltr">
                    </div>
                    ...
                    <div>
                        ...
                        <select x-model="sort_by" @change="fetchData()" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500">
                            ...
                            <option value="balance_desc">الأعلى مديونية</option>
                            <option value="balance_asc">الأقل مديونية</option>
                    ...
                    <form action="{{ route('customers.store') }}" method="POST">
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="space-y-4">
                                ...
                                <div x-data="{ initialAmount: 0, balanceType: 'debit' }">
                                    <input type="hidden" name="balance" :value="balanceType === 'credit' ? -Math.abs(parseFloat(initialAmount) || 0) : Math.abs(parseFloat(initialAmount) || 0)">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-12 gap-2">
                                        ...
                                        <div class="sm:col-span-5">
                                            <select x-model="balanceType" class="w-full px-3 py-2 border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-all text-xs font-bold text-slate-700">
                                                <option value="debit">عليه (مطلوب من العميل)</option>
                                                <option value="credit">له (رصيد دائن للعميل)</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="text-[0.7rem] text-slate-500 mt-1 flex items-center gap-1 font-medium">
                                        <span>الحالة:</span>
                                        <span class="font-bold" :class="initialAmount > 0 ? (balanceType === 'debit' ? 'text-danger-600' : 'text-emerald-700') : 'text-slate-500'" x-text="initialAmount > 0 ? (balanceType === 'debit' ? `العميل مطلوب منه ${Number(initialAmount).toLocaleString()} ج.م` : `العميل له عندنا ${Number(initialAmount).toLocaleString()} ج.م`) : 'الحساب خالص (صفر)'"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
