...
<div class="sm:hidden space-y-3 mb-6">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4 hover:border-slate-200 transition-colors">
        <div class="flex justify-between items-start mb-2">
            ...
            <span class="text-[0.7rem] font-bold text-slate-400 font-mono" dir="ltr">{{ ($purchase->invoice_date ?? $purchase->created_at)->format('Y-m-d') }}</span>
        </div>
        ...
        <div class="flex justify-between items-center pt-2 border-t border-slate-50 mt-2">
            ...
            <div class="flex items-center gap-1.5">
                ...
                @if($purchase->id === (int) $purchase->latest_party_invoice_id && auth()->user()?->hasPermission('purchases.update'))
                ...
                <button type="button" @click="deletePurchaseId = {{ $purchase->id }}; deleteInvoiceNumber = '{{ $purchase->invoice_number }}'; showDeleteModal = true" class="p-1 rounded border border-slate-200 bg-white text-slate-400 hover:text-rose-600 hover:border-rose-200 hover:bg-rose-50 shadow-sm transition-all inline-flex items-center justify-center" title="حذف الفاتورة">
    ...
    <table class="w-full text-center border-collapse whitespace-nowrap">
        ...
        <tbody>
            ...
            <tr class="hover:bg-slate-50/60 transition-colors group">
                ...
                <td class="px-4 py-2.5 text-[0.8rem] text-slate-600 border-b border-slate-50 align-middle text-center font-mono" dir="ltr">{{ ($purchase->invoice_date ?? $purchase->created_at)->format('Y-m-d') }}</td>
