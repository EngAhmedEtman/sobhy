...
<div class="sm:hidden space-y-3 mb-6">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4 hover:border-slate-200 transition-colors">
        <div class="flex justify-between items-start mb-2">
            <div>
                <span class="text-xs font-bold text-primary-700 font-mono">{{ $purchase->invoice_number }}</span>
                <h4 class="text-sm font-bold text-slate-800 mt-0.5">
                    <a href="{{ route('suppliers.show', $purchase->supplier_id) }}" class="hover:underline">{{ $purchase->supplier->name }}</a>
                </h4>
            </div>
            <span class="text-[0.7rem] font-bold text-slate-400 font-mono" dir="ltr">{{ $purchase->created_at->format('Y-m-d') }}</span>
        </div>
        ...
        <div class="flex justify-between items-center pt-2 border-t border-slate-50 mt-2">
            <div>
                ...
                <span class="text-sm font-black text-slate-800" dir="ltr">{{ format_amount($purchase->total_amount) }} <span class="text-[0.65rem] text-slate-400 font-normal">ج.م</span></span>
            </div>
            <div class="flex items-center gap-1.5">
                <button type="button" @click="viewInvoice({{ $purchase->id }})" class="p-1 rounded border border-slate-200 bg-white text-slate-400 hover:text-emerald-600 hover:border-emerald-200 hover:bg-emerald-50 shadow-sm transition-all inline-flex items-center justify-center" title="عرض الفاتورة">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                </button>
                <button type="button" @click="$dispatch('edit-purchase', {{ $purchase->id }})" class="p-1 rounded border border-slate-200 bg-white text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 shadow-sm transition-all inline-flex items-center justify-center" title="تعديل الفاتورة">
                ...
                <button type="button" @click="$dispatch('open-modal', 'delete-purchase-{{ $purchase->id }}')" class="p-1 rounded border border-slate-200 bg-white text-slate-400 hover:text-rose-600 hover:border-rose-200 hover:bg-rose-50 shadow-sm transition-all inline-flex items-center justify-center" title="حذف الفاتورة">
                ...
                <x-delete-modal name="delete-purchase-{{ $purchase->id }}" action="{{ route('purchases.destroy', $purchase->id) }}" title="حذف فاتورة مشتريات رقم {{ $purchase->invoice_number }}" message="سيتم التراجع عن إضافة الكميات إلى المخزن وإلغاء استحقاق المورد الخاص بهذه الفاتورة." />
            </div>
        </div>
    </div>
    @empty
    <div class="bg-white rounded-xl border border-slate-100 p-8 text-center text-sm text-slate-500">
        ...
        لا توجد فواتير مشتريات مطابقة لخيارات البحث.
    </div>
    @endforelse
</div>
    ...
    <table class="w-full text-center border-collapse whitespace-nowrap">
        <thead class="bg-slate-50">
            <tr>
                ...
                <th class="px-4 py-3 text-[0.75rem] font-bold text-slate-500 border-b border-slate-100 uppercase tracking-wide align-middle text-center">الأصناف المشتراة</th>
        ...
        <tbody>
            ...
            <tr class="hover:bg-slate-50/60 transition-colors group">
                <td class="px-4 py-2.5 text-[0.8rem] font-bold text-primary-700 border-b border-slate-50 align-middle text-center font-mono">{{ $purchase->invoice_number }}</td>
                <td class="px-4 py-2.5 text-[0.8rem] text-slate-600 border-b border-slate-50 align-middle text-center font-mono" dir="ltr">{{ $purchase->created_at->format('Y-m-d') }}</td>
                <td class="px-4 py-2.5 text-[0.8rem] font-bold text-slate-700 border-b border-slate-50 align-middle text-center">
                    <a href="{{ route('suppliers.show', $purchase->supplier_id) }}" class="hover:underline hover:text-primary-600">{{ $purchase->supplier->name }}</a>
                </td>
                ...
                <td class="px-4 py-2.5 text-[0.8rem] font-bold text-slate-800 border-b border-slate-50 align-middle text-center" dir="ltr">
                    {{ format_amount($purchase->total_amount) }} <span class="text-[0.65rem] text-slate-400 font-normal">ج.م</span>
                </td>
                <td class="px-4 py-2.5 text-[0.8rem] text-slate-500 border-b border-slate-50 max-w-[150px] truncate align-middle text-center">{{ $purchase->notes ?? '-' }}</td>
