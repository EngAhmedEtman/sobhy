...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => 'إنشاء فاتورة مشتريات']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'إنشاء فاتورة مشتريات']); ?>
     <?php $__env->slot('breadcrumb', null, []); ?> المشتريات > فاتورة جديدة <?php $__env->endSlot(); ?>

    <div class="mb-4 flex justify-end">
        <a href="<?php echo e(route('purchases.index')); ?>" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 text-sm font-bold flex items-center gap-2 transition-all shadow-sm">
            العودة للفواتير
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"
        ...
        <div class="p-4 sm:p-6 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-base sm:text-xl font-black text-slate-800">فاتورة مشتريات جديدة</h2>
                <p class="text-xs text-slate-500 mt-0.5">إضافة مبيعات وإضافتها من المخزن تلقائياً</p>
            </div>
        </div>

        <form action="<?php echo e(route('purchases.store')); ?>" method="POST" class="p-4 sm:p-6">
            <?php echo csrf_field(); ?>
            
            <!-- Supplier & Notes -->
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 pb-6 border-b border-slate-100">
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">اسم المورد <span class="text-danger-500">*</span></label>
                    <select name="supplier_id" required class="w-full px-3 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:border-primary-500 focus:ring-1 bg-slate-50 text-base">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center bg-slate-50 p-4 sm:p-6 rounded-xl border border-slate-100 gap-4">
                <div class="text-center sm:text-right w-full sm:w-auto">
                    ...
                    <p class="text-[0.65rem] text-danger-600 mt-1 font-bold">سيتم إضافة هذا الإجمالي لمديونية المورد فور الحفظ.</p>
                </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('purchasesInvoice', ({ products, suppliers }) => ({
                products: products,
                suppliers: suppliers,
