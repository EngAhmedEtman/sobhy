<x-layouts.app title="إدارة المشتريات">
    ...
    @click="if ($event.target.closest('.ajax-pagination a')) {
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
            ...
            <button @click="$dispatch('create-purchase')" type="button" class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-xs sm:text-sm font-bold flex items-center transition-colors shadow-sm shadow-primary-600/20 w-full sm:w-auto justify-center">
        ...
        <div class="bg-white rounded-xl p-3 sm:p-4 border border-slate-100 shadow-sm mb-4">
            ...
            <div x-show="showFilters" 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2.5">
                    <!-- Supplier Filter -->
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">المورد</label>
                        <select x-model="supplier_id" @change="fetchData()" class="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none focus:border-primary-500">
                    ...
                    <!-- Product Filter -->
                    <div>
                        <label class="block text-[0.72rem] font-bold text-slate-600 mb-1">الصنف المشترى</label>
