...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => 'إدارة المشتريات']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'إدارة المشتريات']); ?>
    <div x-data="{ 
        ...
        viewInvoice(id) { 
         ...
         <?php $__env->slot('breadcrumb', null, []); ?> المشتريات <?php $__env->endSlot(); ?>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-xs sm:text-sm mt-0.5">سجل فواتير المشتريات</p>
                </div>
            </div>
            <button @click="$dispatch('create-purchase')" type="button" class="px-5 py-2.5 bg-primary-600 text-white rounded-xl hover:bg-primary-700 text-sm font-bold flex items-center transition-colors shadow-sm shadow-primary-600/20 w-full sm:w-auto justify-center">
                ...
                <table class="w-full text-center border-collapse whitespace-nowrap">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-[0.75rem] font-bold text-slate-500 border-b border-slate-100 uppercase tracking-wide">اسم المورد</th>
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors group">
                            <td class="px-4 py-3 text-[0.8rem] font-bold text-primary-700 border-b border-slate-100"><?php echo e($purchase->invoice_number); ?></td>
                            <td class="px-4 py-3 text-[0.8rem] text-slate-600 border-b border-slate-100"><?php echo e($purchase->created_at->format('Y-m-d')); ?></td>
                            <td class="px-4 py-3 text-[0.85rem] font-bold text-slate-700 border-b border-slate-100">
                                <a href="<?php echo e(route('suppliers.show', $purchase->supplier_id)); ?>" class="hover:underline hover:text-primary-600"><?php echo e($purchase->supplier->name); ?></a>
                            </td>
                            ...
                            <td class="px-4 py-3 text-[0.85rem] font-bold text-slate-700 border-b border-slate-100" dir="ltr"><?php echo e(number_format($purchase->total_amount, 2)); ?></td>
                            <td class="px-4 py-3 text-[0.8rem] text-slate-500 border-b border-slate-100 max-w-[150px] truncate"><?php echo e($purchase->notes ?? '-'); ?></td>
                            <td class="px-4 py-3 border-b border-slate-100">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" @click="viewInvoice(<?php echo e($purchase->id); ?>)" class="p-1.5 rounded border border-slate-200 bg-white text-slate-400 hover:text-primary-600 hover:border-primary-600 hover:bg-primary-50 shadow-sm transition-all" title="عرض الفاتورة">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                    </button>
                                    <button type="button" @click="$dispatch('edit-purchase', <?php echo e($purchase->id); ?>)" class="p-1.5 rounded border border-slate-200 bg-white text-slate-400 hover:text-warning-600 hover:border-warning-600 hover:bg-warning-50 shadow-sm transition-all" title="تعديل الفاتورة">
                                    ...
                                    <button type="button" x-data @click="$dispatch('open-modal', 'delete-purchase-<?php echo e($purchase->id); ?>')" class="p-1.5 rounded border border-slate-200 bg-white text-slate-400 hover:text-danger-600 hover:border-danger-600 hover:bg-danger-50 shadow-sm transition-all" title="حذف الفاتورة">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.delete-modal','data' => ['name' => 'delete-purchase-'.e($purchase->id).'','action' => ''.e(route('purchases.destroy', $purchase->id)).'','title' => 'حذف فاتورة مشتريات رقم '.e($purchase->invoice_number).'','message' => 'سيتم التراجع عن إضافة الكميات للمخزن وإلغاء المديونية المتعلقة بها.']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'delete-purchase-'.e($purchase->id).'','action' => ''.e(route('purchases.destroy', $purchase->id)).'','title' => 'حذف فاتورة مشتريات رقم '.e($purchase->invoice_number).'','message' => 'سيتم التراجع عن إضافة الكميات للمخزن وإلغاء المديونية المتعلقة بها.']); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.modals.purchase-form','data' => ['products' => $products,'suppliers' => $suppliers]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['products' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($products),'suppliers' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($suppliers)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
