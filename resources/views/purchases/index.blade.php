                ...
                <table class="w-full text-center border-collapse whitespace-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors group">
                            <td class="px-4 py-3 text-[0.8rem] font-bold text-primary-700 border-b border-slate-100">{{ $purchase->invoice_number }}</td>
                            <td class="px-4 py-3 text-[0.8rem] text-slate-600 border-b border-slate-100">{{ $purchase->created_at->format('Y-m-d') }}</td>
                            ...
                            <td class="px-4 py-3 text-[0.85rem] font-bold text-slate-700 border-b border-slate-100" dir="ltr">{{ number_format($purchase->total_amount, 2) }}</td>
                            <td class="px-4 py-3 text-[0.8rem] text-slate-500 border-b border-slate-100 max-w-[150px] truncate">{{ $purchase->notes ?? '-' }}</td>
                            <td class="px-4 py-3 border-b border-slate-100">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" @click="viewInvoice({{ $purchase->id }})" class="p-1.5 rounded border border-slate-200 bg-white text-emerald-600 hover:text-emerald-700 hover:border-emerald-300 hover:bg-emerald-50 shadow-sm transition-all" title="عرض الفاتورة">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                    </button>
                                    <button type="button" @click="$dispatch('edit-purchase', {{ $purchase->id }})" class="p-1.5 rounded border border-slate-200 bg-white text-blue-600 hover:text-blue-700 hover:border-blue-300 hover:bg-blue-50 shadow-sm transition-all" title="تعديل الفاتورة">
                                    ...
                                    <button type="button" x-data @click="$dispatch('open-modal', 'delete-purchase-{{ $purchase->id }}')" class="p-1.5 rounded border border-slate-200 bg-white text-danger-600 hover:text-danger-700 hover:border-danger-300 hover:bg-danger-50 shadow-sm transition-all" title="حذف الفاتورة">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                    </button>
                                    <x-delete-modal name="delete-purchase-{{ $purchase->id }}" action="{{ route('purchases.destroy', $purchase->id) }}" title="حذف فاتورة مشتريات رقم {{ $purchase->invoice_number }}" message="سيتم التراجع عن إضافة الكميات للمخزن وإلغاء المديونية المتعلقة بها." />
                                </div>
                            </td>
                        </tr>
                        @empty
