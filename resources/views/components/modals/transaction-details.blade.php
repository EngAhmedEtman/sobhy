...
@view-transaction.window="viewTransaction($event.detail)">
    ...
    <div x-show="showTransactionDetailsModal" x-cloak class="fixed inset-0 z-[60] overflow-y-auto">
        <div class="flex items-start justify-center min-h-screen pt-10 sm:pt-16 p-4 text-center" @click.self="showTransactionDetailsModal = false">
            <div x-show="showTransactionDetailsModal" x-transition.opacity class="fixed inset-0 transition-opacity bg-slate-900/60 backdrop-blur-sm" @click="showTransactionDetailsModal = false"></div>
            <div x-show="showTransactionDetailsModal" @click.outside="showTransactionDetailsModal = false" x-transition class="relative w-full max-w-2xl p-5 sm:p-6 text-right transition-all transform bg-white shadow-2xl rounded-2xl z-10">
