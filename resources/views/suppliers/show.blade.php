<x-layouts.app title="حساب المورد">
    ...
    @edit-transaction.window="editFromDetails($event.detail)">
        ...
        <div x-show="showReturnModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen p-4 text-center" @click.self="showReturnModal = false">
                ...
                <div x-show="showReturnModal" 
                     ...
                     x-data="{
                         ...
                         submitForm(e) {
                             ...
                             if (this.stockExceeded) {
                                 window.dispatchEvent(new CustomEvent('show-toast', { 
                                     detail: { message: 'الكمية المراد إرجاعها أكبر من الرصيد المتوفر في المخزن', type: 'error' } 
                                 }));
                                 return;
                             }
                    ...
                    <form action="{{ route('suppliers.return', $supplier->id) }}" method="POST" @submit.prevent="if (!selectedId) { productError = true; return; } productError = false; $el.submit();">
                        @csrf
                            ...
                            <div class="md:col-span-7 space-y-4">
                                ...
                                <div class="relative">
                                    ...
                                    <button type="button" 
                                        ...
                                        <span class="text-xs font-bold truncate" :class="selectedId ? 'text-slate-800' : (productError ? 'text-danger-600' : 'text-slate-400')" x-text="selectedProduct ? selectedProduct.name + ' (متوفر بالمخزن: ' + Number(selectedProduct.stock).toLocaleString('en-US') + ' ك)' : 'اختر المنتج المطلوب...'"></span>
        ...
        <x-modals.print-statement :entityId="$supplier->id" type="supplier" />
