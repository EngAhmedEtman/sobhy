            ...
            <thead class="report-repeat-header">
                <tr>
                    <td class="report-repeat-cell">
                        <x-print.header :title="$title" :subtitle="$subtitle" :referenceCode="'SALES-REP-' . date('Ymd-Hi')" />
                    </td>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td class="report-repeat-cell">
                        ...
                        <div class="stats-grid">
                            ...
                            <div class="stat-card">
                                <div class="stat-label">إجمالي المبلغ المحصل (المدفوع)</div>
                                <div class="stat-value success">{{ format_amount($totalPaid) }} ج.م</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-label">إجمالي المتبقي (الآجل)</div>
                        ...
                        <table class="print-data-table">
                            ...
                            <tbody>
                                @forelse($sales as $index => $sale)
                                    @php
                                        $itemsSummary = $sale->items->map(function($i) {
                                            return ($i->product->name ?? 'صنف') . ' (' . format_quantity($i->quantity) . ')';
                                        })->implode(' ، ');
                                        $isPaid = $sale->remaining_amount <= 0;
                                        $isPartial = $sale->paid_amount > 0 && $sale->remaining_amount > 0;
                                    @endphp
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td style="font-weight: 800; font-family: 'Cairo', sans-serif; direction: ltr;">INV-{{ str_pad($sale->id, 6, '0', STR_PAD_LEFT) }}</td>
                                        <td style="font-family: 'Cairo', sans-serif; direction: ltr;">{{ ($sale->invoice_date ?? $sale->created_at)->format('Y-m-d') }}</td>
                                        <td style="font-weight: 700; text-align: right; padding-right: 6px;">{{ $sale->customer->name ?? 'عميل نقدي' }}</td>
                                        <td style="text-align: right; padding-right: 6px; font-size: 9.5px; color: #475569;">{{ $itemsSummary ?: '---' }}</td>
                                        <td style="direction: ltr; font-family: 'Cairo', sans-serif; font-weight: 800;">{{ format_amount($sale->total_amount) }} ج.م</td>
                                        <td style="direction: ltr; font-family: 'Cairo', sans-serif; color: #166534;">{{ format_amount($sale->paid_amount) }} ج.م</td>
                                        <td style="direction: ltr; font-family: 'Cairo', sans-serif; font-weight: 800;" class="{{ $sale->remaining_amount > 0 ? 'danger' : 'success' }}">
                                            {{ format_amount($sale->remaining_amount) }} ج.م
                                        </td>
                                        <td>
                                            @if($isPaid)
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" style="padding: 16px; color: #64748b;">لا توجد فواتير مبيعات مطابقة للبحث أو الفلاتر المحددة</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td colspan="5" style="text-align: right; padding-right: 6px;">الإجمالي العام ({{ $totalInvoices }} فاتورة مبيعات)</td>
