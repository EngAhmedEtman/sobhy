...
<head>
    ...
    <style>
        ...
        *, ::before, ::after { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Cairo', sans-serif !important; }
        ...
        /* Floating action bar for browser preview */
        .no-print-bar {
            ...
            <thead class="report-repeat-header">
                <tr>
                    <td class="report-repeat-cell">
                        <x-print.header :title="$title" :subtitle="$subtitle" :referenceCode="'CUST-REP-' . date('Ymd-Hi')" />
                    </td>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td class="report-repeat-cell">
                        ...
                        <div class="stats-grid">
                            <div class="stat-card">
                                <div class="stat-label">إجمالي عدد العملاء</div>
                                <div class="stat-value">{{ $totalCustomers }} عميل</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-label">إجمالي مطلوب من العملاء (مديونية)</div>
                                <div class="stat-value danger">{{ format_amount($totalDebt) }} ج.م</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-label">إجمالي أرصدة دائنة للعملاء</div>
                        ...
                        <table class="print-data-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 100px;">إجمالي المبيعات</th>
                            ...
                            <tbody>
                                @forelse($customers as $index => $customer)
                                    @php
                                        $salesTotal = \App\Models\Sale::where('customer_id', $customer->id)->sum('total_amount');
                                        $isDebt = $customer->balance > 0;
                                        $isCredit = $customer->balance < 0;
                                    @endphp
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td style="font-weight: 700; text-align: right; padding-right: 6px;">{{ $customer->name }}</td>
                                        <td style="direction: ltr; font-family: 'Cairo', sans-serif;">{{ $customer->phone ?? '---' }}</td>
                                        <td style="direction: ltr; font-family: 'Cairo', sans-serif;">{{ format_amount($salesTotal) }} ج.م</td>
                                        ...
                                        <td>
                                            @if($isDebt)
                                                <span class="badge badge-danger">مديونية</span>
                                            @elseif($isCredit)
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="3" style="text-align: right; padding-right: 6px;">الإجمالي العام ({{ $totalCustomers }} عميل)</td>
